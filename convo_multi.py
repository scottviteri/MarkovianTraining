...
for i in range(num_models):
    ...
    conversations.append([{"role": "system", "content": model_prompt}, {"role": "user", "content": "<Observation> </Observation>"}])
...
def print_messages(messages):
    for message in messages:
        ...
        if role == 'assistant':
            ...
            if action:
                print(f"Action: {action.group(1).strip()}")
        else:
