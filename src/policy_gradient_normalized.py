...
def calculate_answer_log_probs(model, tokenizer, device, reasoning_tokens, answers):
    ...
    avg_log_probs = []
    for i, (mask, probs, start_idx, answer) in enumerate(zip(tokenized_full_prompts.attention_mask, answer_log_probs, answer_start_positions, answers)):
        answer_length = len(str(answer))  # Convert to string in case answer is not already a string
        actual_tokens = mask[start_idx:].sum().item()
        assert actual_tokens == answer_length, f"Mismatch in answer length for index {i}. Expected {answer_length}, got {actual_tokens}"
...
def calculate_losses(unfrozen_avg_log_probs_reasoning_given_question, frozen_avg_log_probs_reasoning_given_question, advantage, use_ppo, ppo_epsilon):
    if use_ppo:
        ...
        policy_loss = (unfrozen_avg_log_probs_reasoning_given_question * -advantage.detach()).mean()
        ppo_ratio = None
        clipped_ratio = None
