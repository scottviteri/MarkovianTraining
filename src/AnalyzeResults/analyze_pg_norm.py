...
def plot_metrics(file_path, window_size=1, output_file='src/AnalyzeResults/pg_norm_plot.png'):
    ...
    if 'PPO Ratio' in metrics:
        axs[3, 0].plot(moving_average(metrics['PPO Ratio'], window_size))
        axs[3, 0].set_title('PPO Ratio (Moving Average)')
        axs[3, 0].set_xlabel('Batch')
        axs[3, 0].set_ylabel('Ratio')

        axs[3, 1].plot(moving_average(metrics['PPO Clipped Ratio'], window_size))
        axs[3, 1].set_title('PPO Clipped Ratio (Moving Average)')
