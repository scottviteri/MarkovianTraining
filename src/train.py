...
def update_weights(
    ...
    with autocast(
        ...
        if prediction_cfg.train_O_given_prev_O:
            ...
            obs_loss = (
                obs_tensor.sum()
                / mkv_attention_mask[:, 1:][:, -cfg.tok_p_pure_obs :].sum()
            )
            ...
            aggregate_loss = sum(
                map(
                    lambda x: x[1] if x[0] else 0.0,
                    zip(
                        [
                            prediction_cfg.train_A_given_AO,
                            prediction_cfg.train_O_given_A,
                        ],
